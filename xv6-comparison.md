...
> 本文档为 EVOLVE 与 xv6 的独立双方案对比文档，原三方对比内容已并入本文并去重，存放在仓库根目录。
>
> 对比对象：
>
...
>
> 数据口径：EVOLVE 数据为 2026-08-15 对仓库源码的实测，统计 `kernel/`、`os-*/`、`user/` 下的 `.rs` 与 `.asm`，排除 `scaffold/` 实验变体、`target/` 和 `node_modules/`；xv6 数据基于 MIT 6.S081 公开课程与 xv6-riscv 公开仓库整理。本校实际开设的 lab 数量、版本年份等仍需以本校老师提供的信息为准。
...
已知教学简化：用户页表中仍包含内核 trap 区的恒等映射（`map_kernel_trap_regions_user`），尚未实现 xv6 那种高地址 `TRAMPOLINE` / `TRAPFRAME` 严格隔离。

xv6 的 trap 设计更工程化：用户态 trap 从映射在高地址的 `uservec`（trampoline）进入，把寄存器保存到每个进程独立的 `TRAPFRAME`，切换 `satp` 后跳转 `kernelvec`；`usertrap` 与 `kerneltrap` 分开处理，UART、磁盘等中断经 PLIC 分发。

| 维度 | EVOLVE | xv6 |
| --- | --- | --- |
| trap 入口 | 单一 `__alltraps` | `uservec` / `kernelvec` 分离 |
| 用户态上下文保存位置 | 内核栈上的 `TrapContext` | 高地址 `TRAPFRAME` |
| 隔离方式 | 用户页表含内核 trap 区恒等映射（教学简化） | 用户页表只映射 trampoline + trapframe |
...
| fork | 深拷贝用户页（无 COW） | 基础版深拷贝；lab 中可扩展 COW |
| exec | 重建地址空间，替换 ELF 映射 | `exec.c` 解析 ELF 并重建页表 |
| 堆扩展 | 固定地址空间区域，无 `sbrk` 体系 | `sbrk` 系统调用增长堆 |
| mmap | Lab6 支持匿名 `mmap` / `munmap`（页对齐、权限位） | 基础内核用 `sbrk`；6.S081 lab 补充文件映射 mmap |
| 隔离强度 | 用户页表含内核 trap 区映射，隔离为教学简化 | trampoline/trapframe 之外用户态无法看到内核映射 |

差异重点：

- EVOLVE 把“帧分配、页表索引、PTE 权限、ELF 段映射”拆成独立 crate 并配 host 单测，非常适合逐步学习；
- xv6 的 `vm.c` 提供了更完整、更接近真实内核的虚存管理，包括 `copyin` / `copyout`、kstack 映射、trampoline 隔离；
...
| fork | `SYS_CLONE` 教学简化版（无 flags），深拷贝地址空间 | 完整 `fork`，深拷贝地址空间（lab 可改 COW） |
...
- EVOLVE 的 wait4 是协作式轮询，xv6 用 `sleep` / `wakeup` 实现真正的阻塞等待，后者是更贴近真实内核的教学模型；
...
- EVOLVE 的接口刻意精简，降低学生理解 ABI 的成本，但部分语义与真实 Linux 不完全一致；
- xv6 的接口更完整、更接近真实系统，适合在理解概念后深入；
...
| FS 分层 | fd 表 + easy-fs inode/数据块层；未单独实现 bcache/log | fd 表 + OFTABLE + icache + bcache + log + inode + 目录层 |
| 硬链接 | `FileIndex` 维护内存别名，未完整跨重启持久化 | 磁盘目录项 + inode `nlink`，持久化 |
| 崩溃一致性 | easy-fs 提供基础读写，无 xv6 式 journal 教学实现 | 自带 log 层（journal）保证事务性 |
...
| 设备文件 | 无 | `mknod` 支持字符/块设备 |
| 控制台文件 | fd 1 特殊处理为 SBI 输出 | console 作为设备文件接入 fd |

差异重点：

- EVOLVE Lab5 的内嵌 FS 让学生先理解“文件抽象”而不被磁盘细节干扰；
- Lab6 接入真实 VirtIO + easy-fs 后，学生能接触磁盘镜像、inode 读写、硬链接与 fstat，但仍保留教学裁剪；
...
- xv6 主要靠运行期 `usertests` 与评分脚本，更能暴露真实运行环境下的边界问题；
...
- xv6-book 对每个机制的讲解深度和工程完整性仍是 EVOLVE 需要学习的目标；
- 建议教学路径：EVOLVE 入门建立脉络，xv6 深化补全工程细节。

## 十三、学习效率评估

> 基于教学设计原理与两套环境的对比推断，标注【待真实学习数据验证】的项需在学生实际使用后补充。

| 效率维度 | EVOLVE 表现 | 相对 xv6 的优势 |
| --- | --- | --- |
| 认知负担 | 低（9978 行、8 组件 crate、单代码库、feature 演进） | xv6 一开始就要面对完整源码树，lab 间相对独立 |
| 反馈速度 | 快（42 项 host 单测 + Rust 编译器即时纠错） | xv6 的段错误等 C 错误诊断更耗时 |
...
1. 用 EVOLVE 入门（8 个 Lab，约 4–5 周）：建立对 OS 核心机制的直观理解和全局脉络。
2. 用本校 xv6 深化（11 个 lab，约一学期）：在已有概念基础上深入工程实现，覆盖网络、mmap 等进阶主题。

理由：EVOLVE 的“渐进式 + 问题驱动”适合打地基，xv6 的“广覆盖 + 工程化”适合建高楼。两者互补，避免初学者一上来就被 xv6 的 C 语言 + 庞大代码量劝退。

## 十四、局限与改进方向

诚实说明 EVOLVE 的局限（不回避短板）：

1. **覆盖广度有限**：8 个 lab 不含网络、完整 mmap 文件映射、GPU 等 xv6 进阶主题。
2. **系统语义仍是教学子集**：Lab6 已接入 VirtIO/easy-fs 磁盘文件系统和匿名 `mmap/munmap`，但硬链接别名未完整跨重启持久化，也未覆盖文件映射、共享映射和 COW。
3. **调度与隔离仍是教学实现**：具备时间片、轮转、stride 和阻塞同步，但不覆盖多核调度与严格的高地址 trampoline 隔离。
4. **未经大规模教学验证**：自动回归和人工流程试用可以证明工程闭环，学习效果提升仍需真实学生对照数据验证。
...
- 建议学习顺序：先完成 EVOLVE Lab1–Lab8 建立全局脉络，再进入 xv6 深化页表、trap、FS、COW、网络等主题。

## 十七、数据口径与变更记录

> EVOLVE 数据的采集方法与目录口径见文档开头；下表保留历次采集版本。

| 日期 | 变更 |
| --- | --- |
| 2026-06-23 | 初版（Lab1–5，1882 行，9 测） |
| 2026-07-25 | Lab6–8 合入后更新：8 lab、8353 行、40 host 测、8 组件 crate |
| 2026-08-15 | 复核：10 个 workspace 包、72 文件、9978 行、42 host 测；整理为独立 xv6 对比文档 |
