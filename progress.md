...
- [一、项目概览与团队分工](#一项目概览与团队分工)
- [二、开发进度安排](#二开发进度安排)
- [三、当前进度与交付状态](#三当前进度与交付状态)
- [四、每日开发进度记录](#四每日开发进度记录)
...
| 里程碑                                 | 达成日期  | 验收证据                        |
| ----------------------------------- | ----- | --------------------------- |
| Lab1 裸机内核可运行（`Hello, OS!`）          | 06-21 | 06-21 记录                    |
| Lab1–Lab5 五档 feature 全部 QEMU 通过     | 06-26 | 06-26/06-27 交叉回归记录          |
| 24 项组件 host 单元测试全绿                  | 06-26 | 06-26/06-27 记录              |
| clippy `-D warnings` 全 workspace 通过 | 06-27 | 06-27 终验记录                  |
| 参考环境 5 章 exercise checker 全绿        | 06-27 | docs/reference-report.md    |
| Web 学习手册（VitePress）构建通过             | 06-27 | handbook 记录                 |
| 初赛材料与学习手册交付                         | 06-30 | 06-30 收尾记录                  |
| 学生工作台与可信运行闭环                        | 07-30 | C0/C1、Trace Viewer 记录       |
| 评分 v2 与教师复核留痕                       | 08-02 | 评分与复核记录                     |
...
- 文档中的命令与已通过的本机验证流程一致；路径说明与 `activate-os-env.ps1` / `environment_setup.md` 一致。
...
## 2026-06-21 - Task: os-sbi + 构建复核 + 组件脚手架
...
- `os-lab/os-context/src/lib.rs` 等 5 个组件 crate：增加后续占位类型/trait/常量。
...
- 复核团队教学定位：依据 `os-lab/docs/architecture.md`（"Lab1 启动流程（当前已实现）"及"组件 crate 待后续填充"）与代码事实（lab1 五文件完整无 TODO、lab2-5 六文件为 4 行空骨架），确认 lab1 定位为"读+跑+小修改"的入门起跑线，lab2-5 才是学生动手实现的部分。
...
## 2026-06-21 - Task: 实验文档充实（overview + lab1）
...
- lab 文档初稿任务现已从「占位」升级为「实质完成」，lab1 已满足 plan 要求的每个实验必含项（问题场景、背景知识含 mermaid、实验任务、验证方法、AI 提问模板、习题与答案）。
...
- `os-lab/`：新增自研教学实验环境 workspace 根目录及全部初始骨架。
- `os-lab/kernel/`：Lab1 可运行内核与 feature gate 主体。
- `os-lab/os-*/`、`os-lab/user/`：组件与用户态占位 crate（待后续填充）。
...
- `progress.md`：追加本轮记录（用户所称 `process.md` 即本文件）。
...
- 扩充 `docs/os-lab_verify.md`：覆盖全流程（环境激活、编译检查、host 单元测试、Lab1/Lab2 QEMU 运行、成功标准、一键复制命令块、常见问题）。

### Testing

- 文档命令与本机已通过验证流程一致（`cargo test` host triple、`cargo check/run lab2`）。

### Notes

- `docs/os-lab_verify.md`：补全 Lab2 正式验证步骤，移除「可选」表述；新增 Linux/macOS host triple 与一键复制块。
- 回滚方式：`git checkout docs/os-lab_verify.md`。

## 2026-06-22 - Task: os-context + os-syscall + user 测试程序
...
- 更新 `os-lab/tests/README.md` 验证小节；`docs/os-lab_verify.md` 补充 Lab2 可选步骤。
...
## 2026-06-22 - Task: 按学生视角走查 lab1 并修复全部完成度问题（P0/P1/P2）
...
## 2026-06-22 - Task: lab2 文档 + AI 协作模板
...
- `os-lab/docs/ai-collaboration.md`：新增，AI 协作记录模板（plan 对应任务）。
- `progress.md`：追加本轮记录。
...
## 2026-06-22 - Task: 三人完成情况核查 + Lab2 trap/调度/加载
...
| **B** | `rust-toolchain.toml`、`.cargo/config.toml`、链接脚本、SBI 基础 | ✅ 已完成（超额） | 复核 linker/build.rs；新增 `os-sbi` crate；后续组件占位                                                    |
| **C** | `labs/overview.md`、`labs/lab1-bare-metal.md`           | ✅ 已完成     | 两文档已充实；另增 `labs/answers/lab1-answers.md`                                                       |


**总体验收**：`cargo run -p kernel --features lab1` 输出 `Hello, OS!` 并正常退出 —— **通过**。
...
- 为打通 lab2 端到端验证，临时补充 `user/` 下 hello/power/yield 测试程序（属后续范围，待正式接手完善）。
...
## 2026-06-23 - Task: os-alloc/os-vm 正式接手 + 堆分配器 + 单元测试 + 验证文档
...
- `cargo test -p os-context -p os-syscall --target x86_64-pc-windows-msvc`：7 项全部 `ok`（回归）。
...
## 2026-06-23 - Task: lab3 文档 + 三方对比数据采集
...
- 新建 `os-lab/docs/comparison-data.md`：用脚本采集的三方对比原始数据，供后续 `docs/comparison.md` 三方对比报告使用。已采集自研 os-lab（8 crate/1882 行/9 测试）和参考 tg-rcore-tutorial（29 crate/36455 行/0 测试）的定量数据；本校环境数据标注为待调研补充。
...
- `os-lab/docs/comparison-data.md`：新增，三方对比原始数据（plan 对应任务：开始收集三方对比数据）。
- `progress.md`：追加本轮记录。
- 本轮严格遵守成员 C 文件边界（仅改 os-lab/labs/、os-lab/docs/、progress.md），未触碰成员 A 的 kernel/src/、成员 B 的 os-*/、user/。
...
## 2026-06-23 - Task: Lab3 虚存集成（内核 + os-alloc/os-vm）
...
**已知简化（相对计划）**：当前为共享内核页表 + 复用 `0x80400000` 用户槽位按序加载 `.bin`（非每任务独立 satp、非 ELF PT_LOAD）；后续可再演进。
...
**第三节总体验收**：`cargo run -p kernel --features lab2` 加载 3 个用户程序，syscall 正常，关键输出含 `409684505`、`Power check ok`、`Yield round`、`All user apps exited.` —— **通过**。

**遗留缺口（不阻断验收，后续可顺带消化）**

- yield 在批处理调度下仅输出 1 轮 `Yield round` 即进入下一 app（文档已诚实标注）。
- `task.rs` 仍使用单一物理槽 `0x80400000` 直拷二进制，无独立用户页表（属后续范围）。
- kernel 编译有 dead_code/static_mut_refs 等 warning，未阻断运行。

**后续任务已写入计划**（`.cursor/plans/自研os教学实验环境.plan.md`）：`mm.rs` 虚存集成、每任务独立地址空间、ELF 加载、satp 切换；前置依赖成员 B 交付 `os-alloc`/`os-vm`。
...
- `cargo check -p kernel --features lab3`：可编译（`mm.rs` 仍为占位 `init()`，符合未开工状态）。

### Notes

- `progress.md`：追加核查结论；修复顶部 rebase 冲突标记。
- `.cursor/plans/自研os教学实验环境.plan.md`：已完成 todo 标为 completed；新增详细任务清单。
- `os-lab/docs/architecture.md`：补充 Lab2 已实现状态与 待集成说明。
- 回滚方式：`git checkout progress.md .cursor/plans/自研os教学实验环境.plan.md os-lab/docs/architecture.md`。

## 2026-06-24 - Task: os-fs crate + 组件测试 + lab5 用户态/验证
...
## 2026-06-24 - Task: Lab5 文件系统 + 管道同步

### What was done

- 实现 `[kernel/src/fs/embedded.rs](os-lab/kernel/src/fs/embedded.rs)`：每进程 fd 表、内嵌只读文件 `testfile`、`sys_openat`/`sys_read`/`sys_close`、pipe fd 与 `sys_write` 写管道。
...
- 成员 B 未交付 lab5 用户态/`os-fs` 实现，按此前惯例临时补齐验收用 `fs_test`/`pipe_test` 及 `user/syscall.rs` lab5 包装、`os-syscall` 的 `SYS_PIPE`（待 B 正式接手）。
...
## 2026-06-24 - Task: user 进程测试程序 + lab4 验证文档
...
- 更新 `[os-lab/tests/README.md](os-lab/tests/README.md)` 与 `[docs/os-lab.md](docs/os-lab.md)`：新增 Day4/Lab4 验证节、第 12 节一键复制块、Day4 验收勾选清单及常见问题。
...
- ownership 转移：lab4 用户态代码由成员 A 临时补齐，本轮由成员 B 正式接手并文档化；`pipe` 测试程序留待后续。
...
- 三方对比速览表的自研/参考两列沿用已采集的脚本统计数据（1882 行/29 crate 等），本校列用 xv6 公开数据，三列口径一致可对比。
...
- 重要定位：xv6-riscv 是世界级成熟教学内核，三方对比应定位为"差异化互补"而非"替代"——自研优势在 Rust 内存安全、精简架构（6 crate）、问题驱动引导，与 xv6 的 C/广覆盖/经典文档形成互补。这个定位将指导后续 comparison.md 的写作基调。
- 回滚方式：`git checkout os-lab/docs/comparison-data.md progress.md`。

## 2026-06-24 - Task: lab4 文档 + 本校环境调研框架
...
- `os-lab/docs/comparison-data.md`：扩充本校环境调研框架（plan 对应任务：本校教学环境调研与数据整理）。
- `progress.md`：追加本轮记录。
- 本轮严格遵守成员 C 文件边界（仅改 os-lab/labs/、os-lab/docs/、progress.md），未触碰成员 A 的 kernel/src/、成员 B 的 os-*/、user/。
- 本校环境数据因无法自动获取，已搭建调研框架并列出待核实项，未编造任何数据（遵守"信息不足不得猜测"原则）。建议后续向课程老师核实，并在 comparison.md 中填入。
...
## 2026-06-24 - Task: fork/exec/wait 进程管理
...
## 2026-06-24 - Task: 缺口补全（每任务独立地址空间 + ELF 加载）

### What was done

补全计划内未落地项：每任务独立 `MemorySet`/`user_token`、ELF PT_LOAD 加载、`satp` 切换与 trap 返回用户态路径。
...
## 2026-06-24 - Task: 三人完成情况核查 + 任务编排

### What was done

**完成情况核查（对照计划目标、分工与详细清单）**
...
| **C** | `labs/lab3-memory.md`、三方对比数据采集      | ✅ 已完成    | 实验指导 + 答案 + `comparison-data.md`（自研 vs 参考定量数据已采集；本校环境待后续调研）                                                                 |
...
**第三节总体验收**：`cargo run -p kernel --features lab3` 输出含 `lab3: virtual memory ready`、`409684505`、`Power check ok`、5 次 `Yield round`、`All user apps exited.`，exit code 0 —— **通过**（接受已知简化）。

**遗留缺口（不阻断验收，后续可消化）**

- 每任务独立地址空间、ELF 段加载、trampoline 高地址映射（计划内未完全落地项）。
- `process.rs` 仍为占位，lab4 批处理链（`sys_exit` 加载下一 app）待后续重构。
- kernel / 组件 crate 有 `static_mut_refs` 等 warning，未阻断运行。

**后续任务已写入计划**（`.cursor/plans/自研os教学实验环境.plan.md`）：`process.rs` PCB/进程树、`sys_fork`/`sys_execve`/`sys_wait4`、调度重构、移除批处理链；前置依赖成员 B 交付 `fork_test`/`exec_test`。
...
- `cargo check -p kernel --features lab4`：可编译（`process.rs` 占位，符合未开工状态）。

### Notes

- `progress.md`：追加核查结论与任务编排。
- `.cursor/plans/自研os教学实验环境.plan.md`：已完成 todo 标为 completed；新增详细任务清单。
- `os-lab/docs/architecture.md`：更新 Lab3 完成状态、新增 Lab4 待开工说明，移除过时「Lab3 待集成」段落。
- 回滚方式：`git checkout progress.md .cursor/plans/自研os教学实验环境.plan.md os-lab/docs/architecture.md`。

## 2026-06-25 - Task: 三方对比报告 + 学习效率评估 + AI 协作记录完善

### What was done

- 编写 `os-lab/docs/comparison.md`：完整的三方对比分析报告（plan 核心任务）。含三环境概览（mermaid）、定量对比（规模/架构/实验/测试指标表）、定性分析（学习路径清晰度/上手难度/文档友好度）、自研环境 5 大差异化创新点（mermaid）、学习效率评估（理论分析 + 预期学习成果 + 与 xv6 互补建议）、局限与改进方向、结论。所有定量数据基于 comparison-data.md 已采集的真实数据，定性分析有理有据。
...
## 2026-06-25 - Task: Lab1–Lab5 交叉回归 + 验证文档

### What was done

- 在 `os-fs` 内核集成后，执行 Lab1–Lab5 全量交叉回归：workspace 编译检查、24 项 host 单元测试、lab5→lab1 QEMU 倒序运行、`cargo check` lab1–lab5 feature 全覆盖。
...
- 更新 `[docs/os-lab.md](docs/os-lab.md)`：验证总览表补充 Day4/Day5/Day6；第 13 节 os-fs 集成表述修正；新增第 16 节 Day6 一键块与勾选清单；FAQ 补充 PowerShell `$ErrorActionPreference` 说明。
...
- `progress.md`：追加本轮记录。
- 已知缺口（不阻断交付）：lab4 QEMU 缺 `fork_test pass`，属内核 `getpid`/wait 路径，需在终验前修复；勾选清单中已标注 lab4 以 parent/child + `All processes exited.` 为最低通过线。
- 回滚方式：`git checkout os-lab/tests/README.md docs/os-lab_verify.md progress.md`。

## 2026-06-25 - Task: os-fs 接入 + clippy 修复 + 架构文档
...
- `kernel/fs.rs` 未调用 `os-fs` crate（双份 `testfile` 表；feature 已声明 `dep:os-fs`）——后续可选接入 `EmbeddedFs::default_fs()`。
...
- `os-alloc`/`os-vm` 有 `static_mut_refs` 等 warning，未阻断运行；clippy 全绿留待后续。

**后续任务已写入计划**（`.cursor/plans/自研os教学实验环境.plan.md`）：clippy 审查、crate 发布条件检查、architecture 文档完善、可选 `os-fs` 接入、配合 B 交叉回归。
...
- `progress.md`：追加核查结论与任务编排。
- `.cursor/plans/自研os教学实验环境.plan.md`：已完成 todo 标为 completed；新增详细任务清单。
- `os-lab/docs/architecture.md`：Lab5 完成态、syscall 数据流 mermaid、待办与验证命令。
- 回滚方式：`git checkout progress.md .cursor/plans/自研os教学实验环境.plan.md os-lab/docs/architecture.md`。

## 2026-06-25 - Task: lab5 文档 + 5 组文字习题 + xv6 数据复核
...
- 新建 `os-lab/labs/exercises/` 目录：编写 5 组文字类习题（plan 对应任务）。含 README.md 索引 + lab1-5 各一个习题文件，每个 3-5 道概念理解题（不要求写代码，考察对核心概念的掌握），部分答案指向各 lab 文档的"思考题与参考答案"节，部分是新题。
...
- `os-lab/labs/exercises/README.md` + `lab1-exercises.md`...`lab5-exercises.md`：新增，5 组文字类习题（plan 对应任务）。
- `os-lab/docs/comparison-data.md`：修正 2 处过时数据（自研 labs 数、对比表实验数标注）。
- `progress.md`：追加本轮记录。
- 本轮严格遵守成员 C 文件边界（仅改 os-lab/labs/、os-lab/docs/、progress.md），未触碰成员 A 的 kernel/src/、成员 B 的 os-*/、user/。
- 至此 plan 对应任务全部完成（lab5 文档 + 5 组习题）。5 个 lab 的实验指导文档（lab1-5）+ 5 个答案文件 + 5 组习题全部就绪，os-lab 的教学文档体系成型。
...
## 2026-06-26 - Task: clippy 修复 + 新人路径复验 + 终验回归
...
- 执行终验回归：`cargo check --workspace`、B 域 clippy、lab5→lab1 倒序 QEMU、`cargo check` lab1–lab5 feature。
...
- 终验倒序 QEMU（lab5→lab1）：全部 exit 0，关键输出与终验回归表一致。
...
- `progress.md`：追加本轮记录。
- 至此 plan 对应任务全部完成（clippy 修复 + 新人复验 + 终验文档 + progress）。本域任务全部闭环。
...
## 2026-06-26 - Task: 设计总结报告 + overview 修正 + 文档完整性自查

### What was done

- 编写 `os-lab/docs/design-report.md`：完整的设计总结报告（plan 核心任务）。含设计思路与目标（三大痛点+三目标+三大架构决策，含 3 张 mermaid）、与 AI 合作的实现过程（三人协作模型+AI 各环节作用表+四条 AI 协作经验，含 1 张 mermaid）、学习效果评估（教学覆盖度三大主题图+学习效率四维度分析+预期学习成果+与 xv6 互补建议，含 2 张 mermaid）、创新点与差异化价值、局限与改进方向、按评审维度（创新性30%/完整性20%/代码质量25%/文档完整性25%）的自查表、总结。
...
- 至此 plan 对应任务全部完成（设计报告 + 文档自查 + progress 更新）。教学文档任务全部闭环：overview+lab1、lab2+AI模板、lab3+对比数据、lab4+本校调研、lab5+5组习题、三方对比报告+学习效率、设计报告+文档自查。
...
## 2026-06-26 - Task: 全流程回归 + README 终稿 + 评审自查

### What was done

- 执行 Lab1–Lab5 全流程 QEMU 回归（`cargo run -p kernel --features labN --release`），五档 feature 均 exit 0，关键输出与计划验收一致。
- 确认此前修复的 lab4 `fork_test pass` 在终验回归中稳定通过（`I am parent`/`I am child`、`waited pid done`、`fork_test pass`）。
- 更新 `os-lab/README.md`：新人 5 分钟上手路径、实验列表与文档索引、按 Lab 的 QEMU/单元测试命令、与参考环境差异摘要（README 终稿）。
- 更新 `os-lab/docs/architecture.md`：补充 全流程回归结果表。
...
- `os-lab/README.md`：终稿（实验列表、测试命令、文档索引、新人路径）。
- `os-lab/docs/architecture.md`：回归结果表。
- `progress.md`：追加核查结论与本轮记录。
- 回滚方式：`git checkout os-lab/README.md os-lab/docs/architecture.md progress.md`。

## 2026-06-26 - Task: 三人完成情况核查

### What was done

对照计划第三节总目标与第四节分工，逐项核查仓库现状并汇总结论。
...
| `docs/comparison.md`（三方对比 + 学习效率） | ✅   | 三方对比与学习效率交付                                                                       |
| `docs/ai-collaboration.md`        | ✅   | lab1–lab5 示例记录完整                                                                  |
| Markdown 链接有效、mermaid 可渲染         | ⚠️  | `labs/overview.md` 实验列表仍标 lab2–5「待编写」（文档滞后，实际文件已存在）；属后续范围，不阻断验收                   |
...
**总体验收结论：通过**（接受已知遗留：`overview.md` 链接文案滞后；`cargo clippy --all -D warnings` 因 B 域 `os-alloc`/`os-context` 未全绿，已文档化例外；`design-report.md` 属后续任务）。

### Testing

- 核查基于既有 `progress.md` 记录与本轮文件存在性/内容抽检；QEMU 全量回归在终验任务中复跑确认（见上一条记录）。

### Notes

- `progress.md`：追加三方核查结论。
- 遗留项：`labs/overview.md` 待更新链接文案；`docs/design-report.md` 待 C 编写；`cargo clippy --all -D warnings` 待 B 域消化（非 A 阻塞项）。
...
## 2026-06-27 - Task: os-lab 终验收尾（clippy 全绿 + 回归）
...
- 终验回归：24 项 host 单测通过；lab5→lab1 QEMU 关键输出均符合验收标准。
...
- `docs/os-lab_verify.md`：清单更新为 clippy 全绿。
...
- **恢复团队 process**：重新纳入正式实施计划，个人 `.local.md` 仍保持忽略；标记三位成员第一周完成项与尚未完成的共同纵向验收，并在本进度总览补充二期职责。
...
## 2026-07-28 - Task: Lab2 正文增补与变体/UI 文案
...
## 2026-07-28 - Task: Lab2 教学规格与量规草案（M0）
...
## 2026-07-29 - Task: 提交 Trace Viewer

### What was done

- 将本轮 Trace Viewer 交付（TraceViewer/TraceTrapView/TraceTimelineView/useTracePlayback、LabWorkspace 接入、workbench-ui.md 与 visualization/README.md 更新、progress.md 记录）一并提交。
...
## 2026-07-29 - Task: Trace Viewer（Trap 时序 + 任务时间线 + 播放/过滤/跳源码/插入报告）
...
## 2026-07-29 - Task: M1 真实文件状态与编译诊断闭环
...
- M0 第一周基线已经冻结；本轮完成 M1 文件状态、诊断持久化与多用户隔离主链。Trace Viewer 的查询与交互仍属于后续工作。
- 中文文件名 Markdown 保持现有工作区状态，不纳入本轮实现范围。

## 2026-07-29 - Task: 教学 IDE MVP（多标签 / 文件状态 / Problems / 统一上下文）
...
- **Problems 页签接入真实数据**：`ProblemsPanel.vue` 重写，按 `runId` 调 `GET /runs/:id/diagnostics`（待提供），404 保持可信空态（不 mock），200 渲染诊断列表（级别/文件:行/消息），点击 emit `jump` → `LabWorkspace.onProblemJump` → `openAtLine`。
...
- 纯前端实现，未新增后端接口（`/fs/status`、`/runs/:id/diagnostics` 属后续任务）；接口提供后前端即生效，无需再改。
...
- **M0 状态**：第一周个人项与三项周末共同评审均完成，下一阶段进入 M1 教学 IDE MVP。
...
- 正式实施计划在本轮开始前已处于工作区删除状态，本轮未恢复或覆盖该既有变更；本地 `.local.md` 副本已同步第一周完成状态。
...
## 2026-07-29 - Task: 对照 12 周计划补齐教学规格交付

### What was done

- 在已完成的第 1–3 周 Lab2 教学包基础上，补齐后续周次的**可单人交付**产物：Lab3 样板包；可视化视图规格与 OPRE；Lab2/3 检查点与迁移题；量规 T1/T2；标注轨迹扩至 20 条；教师复核门控；Lab 创建模板与审核清单；Lab2 补救变式与 Lab3 debug 规格及难度评估；M5 试用协议。
...
## 2026-07-30 - Task: C0-C1 基线与可信 Trace 证据链
...
## 2026-07-31 - Task: 教学侧全部分项交付

### What was done

- 收口成员 A 全部分项：缺口表、Lab2 `checkpoints.yaml`（requiredEvidence/hintLevel/refusePattern）、5 条导师金标准对话、量规 v2 冻结版、知识路径/OPRE 定稿文案、补救变式装箱与教师验收勾选、5 人可用性最小脚本。
- 更新 `MEMBER-A-DELIVERABLES.md` 与既有草案的交叉链接（指向冻结版/YAML）。

### Testing

- 文档交叉链接与清单自检：`lab-packages/MEMBER-A-7DAY/README.md` 交付落点齐全；`checkpoints.yaml` 覆盖 C2-1…C2-5；金标准 JSON 含 3 拒 + 2 放行；`rubric-v2-frozen.md` 含 14 细项 + H1–H6；remedial `TEACHER_ACCEPTANCE` 教学项已勾选。
...
## 2026-07-31 - Task: 实验报告面板改造（格式下拉 / 图片库 / 上传修复）
...
## 2026-07-31 - Task: 实验报告：自由编写、插图、附件与教师可配版式（基线）
...
- **文档同步**：更新 C0-C7 使用指南中的导师入口与 Trace 路径，补充当前窗口操作和刷新保留行为。
...
- **文档**：`workbench-ui.md` 补充引用跳转契约；`day1-workbench-audit.md` 将 G2 及后续堵点标为已落地。
...
- 未做（属后续）：评分细项 UI、教师复核、Lab 向导、OPRE/知识路径条。

## 2026-08-01 - Task: TutorEvidenceBar
...
- **文档**：`workbench-ui.md` 补充证据条契约；`day1-workbench-audit.md` 将证据条标为已落地，堵点改为引用跳转。
...
- 未做（属后续）：消息内 `run:`/`trace:` 点击跳转、完整提示阶梯文案与拒答态 UI。

## 2026-08-01 - Task: 工作台空态与文档核对

### What was done

- **核对清单**：新建 `os-lab/handbook/docs/day1-workbench-audit.md`，记录空态/mock 核查结论、与 `workbench-ui.md` 不一致项（D1–D6）、挂后续的规格缺口（G2/G7/G8、TutorEvidenceBar），以及本轮已修正文案。
- **文档同步**：重写 `workbench-ui.md` 布局——工作区底栏为终端/Problems/测试结果，Trace 在学习支持页签；顶栏开关恢复、无折叠 rail；Problems API 写明 `GET /run/diagnostics`；ManualPane 职责改为手册（知识路径注明后续）。
...
- **明日堵点**：落地 `TutorEvidenceBar`，消费 chat 已有 `tutorState`（阶段 / 证据 / 下一步）。
...
## 2026-08-02 - Task: 添加到对话：可选范围 + 点击溯源
...
## 2026-08-02 - Task: 工作台「添加到对话」+ 清理临时调度探针
...
## 2026-08-02 - Task: LabCreateWizard
...
- **文档**：`workbench-ui.md`、`day1-workbench-audit.md` 同步契约。
...
- 计划完成标准：不必手改多处配置即可走通 lint → dry-run → 发布；学生领取依赖既有 access/scaffold + 教学安排。
...
## 2026-08-02 - Task: 复核改分 + OPRE + 知识路径
...
- **文档**：`workbench-ui.md`、`day1-workbench-audit.md`、`teacher-report.md` 同步契约。
...
- 计划完成标准：教师能改一条分并留痕；OPRE/路径在页面有入口。Lab 向导属后续。

## 2026-08-02 - Task: 缺口收口
...
- **CI/演示**：新增 GitHub Actions `os-lab-ci` 和统一命令 `npm run test:day7`；补充 3–5 分钟全链路演示清单与工程预演记录。
- **构建收口**：将核查文档中两个越出 VitePress 内容根的相对链接改为仓库链接，恢复链接检查和完整构建。
...
- 计划完成标准已满足（学生得分区 + TeacherReport 细项/证据跳转；教师与学生同证据链）。
- 未入复核队列的评价教师端不可见（无 `GET /teacher/assessments`）；属可信空态，不造假（依赖 C 的 reviews API）。
- 未做（后续）：教师改分留痕、OPRE/知识路径条、Lab 向导。
...
- 补记工程收口：完成 Lab Factory CLI、Tutor Chat 证据引用归属校验、GitHub Actions `os-lab-ci` 与 `npm run test:day7` 链路；CI 使用同一套本地测试命令，避免远端单独维护验证逻辑。
...
- 三个无关未跟踪 Markdown 文件继续保持未跟踪：`Lab手册复核指南.md`、`教师使用指南.md`、历史实施计划文件副本。
...
- 新建 `plan.md`，把 Top3 摩擦、窄屏不崩、文档与实现一致拆成逐项勾选清单，并逐项更新完成状态。
...
- **文档对齐**：更新 `handbook/docs/workbench-ui.md` 的终端（交互式 xterm / 多会话 / 非 PTY）、AI 导师（悬浮窗口）、Trace 页签与测试结果关键断言描述；更新 `handbook/docs/day1-workbench-audit.md` 复核结论。
...
## 2026-08-03 - Task: 实验手册/工作台 UI 收口 + 链路验收
...
- 未重跑真实 QEMU lab2 发布；如需实机验收证据，需在本地按手册跑一次教师发布 → 学生领取。
