...
//! 1. 先复现并记录输出：谁把 `I am child` 打印了两次？父进程为什么走进了子分支？
//! 2. 假设：父进程拿到的 `fork` 返回值是不是也被设成了 0？「父得子 PID、子得 0」
//!    这两条不变式在 `sys_fork` 里各对应哪一行？
//! 3. 沿 `sys_fork` 对照：子进程用 `TrapContext::set_return_value(0)`，父进程用什么
//!    返回 `child_pid`？
//! 4. 修复后必须看到 `I am parent`、`I am child`、`fork_test pass`、
//!    `All processes exited.`，不要只看「进程都退出了」。
//!
...
pub fn sys_fork(cx: &mut TrapContext) -> isize {
    ...
    PROCESS_MANAGER.with(|pm| {
        ...
        #[cfg(not(feature = "lab8"))]
        {
            ...
            child_pcb.status = ProcessStatus::Ready;
            // PLANTED BUG: 父进程必须返回 child_pid；子进程 a0 才是 0。
            0 as isize
        }
    })
}
