//! 【Lab8 任务：debug】互斥测例卡住或看不到 `mutex_test pass`。
//!
//! 现象：`make test-lab8` 时 `threads_test` 仍可能通过，但 `mutex_test` /
//! `condvar_test` 长时间无进展，或缺少对应 `pass` 行。
//!
//! 按「现象 → 假设 → 最小实验 → 证据 → 结论」排查：
//! 1. 先复现：有没有 `threads_test pass`？卡在哪一行之后？
//! 2. 假设：拿不到锁的线程是否已 Blocked？持锁线程 `unlock` 后有没有把等待者
//!    重新入队（`re_enque`）？
//! 3. 沿 `sys_mutex_unlock`：`mark_mutex_handoff` 之后是否还调用了 `re_enque`？
//! 4. 修复后必须看到 `mutex_test pass`、`condvar_test pass` 及全链其余断言。
//!
...
pub fn sys_mutex_unlock(mutex_id: usize) -> isize {
    ...
    if let Some(tid) = waking_tid {
        ...
        // PLANTED BUG: 只标记 handoff，没有把等待者重新入就绪队列。
        // 等待线程永远停在 Blocked，mutex_test / condvar_test 往往卡住。
        // re_enque(tid);
