    ...
    while va < APP_BASE_ADDRESS + APP_REGION_SIZE - USER_STACK_SIZE {
        if let Some(perm) = user_space.leaf_perm(va) {
            if perm.contains(MapPermission::U) {
                let mut restricted = MapPermission::R;
                if perm.contains(MapPermission::W) {
                    restricted = restricted.union(MapPermission::W);
                }
                if perm.contains(MapPermission::X) {
                    restricted = restricted.union(MapPermission::X);
                }
                let ppn = user_space.translate(VirtAddr(va)).expect("mapped ELF page");
                user_space.page_table_mut().remap(VirtAddr(va).floor(), ppn, restricted);
            }
        }
        va += PAGE_SIZE;
    }
