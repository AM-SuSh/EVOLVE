//! 【Lab6 任务：debug】硬链接测例失败，常见看不到 `Test link OK!`。
//!
//! 现象：`make test-lab6` 时 `file_test` 仍可能通过，但 `link_test` 报
//! `fstat nlink mismatch` 或缺少 `Test link OK!`。
//!
//! 按「现象 → 假设 → 最小实验 → 证据 → 结论」排查：
//! 1. 先复现：有没有 `file_test pass`？有没有 `Test link OK!`？
//! 2. 假设：用户测例对 nlink 的预期是否合理？若用户侧参考实现正常，
//!    内核 `DiskFs::link` 在共享 `FileMeta` 时有没有执行 `nlink += 1`？
//! 3. 沿 `sys_linkat` → `DiskFs::link` 核对：`aliases.insert` 前后是否 bump 了 nlink？
//! 4. 修复后必须看到 `file_test pass`、`Test link OK!` 及全链其余断言。
//!
...
impl DiskFs {
    ...
    fn link(&self, src: &str, dst: &str) -> isize {
        ...
        // PLANTED BUG: 只把 dst 挂到同一份 FileMeta，没有增加 nlink。
        // 对比手册：硬链接应共享 ino，且 nlink 随目录项数量 +1。
        // 用户测例 link_test 期望链接后 nlink == 2，这里漏 bump 就会 mismatch。
        // meta.lock().nlink += 1;
