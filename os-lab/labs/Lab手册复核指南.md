...
2.3讲解：请仔细理解这里“对 SBI 调用进行封装”的含义：**将底层的寄存器调用约定转换成上层可以直接使用的 Rust 函数接口**。

这里还需要强调一个容易混淆的点：
console.rs 的封装对象：格式化输出
os-sbi 的封装对象：SBI 调用协议
OpenSBI 的封装对象：具体平台和硬件差异



BSS 清零并不是为了专门支持 `println!`。
...
**低特权级通过 `ecall` 请求高特权级服务**的基本思想





出题类型：绘制流程图（可以是给mermaid然后挖空的形式）
