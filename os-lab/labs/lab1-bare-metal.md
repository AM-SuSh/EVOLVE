...
> 相关教材理论：
>
>  [第 2 章 · 操作系统简介（P16）](/downloads/ostep-zh.pdf#page=16)
>
>  [第 6 章 开头引言（P49）](/downloads/ostep-zh.pdf#page=49)
...
OSTEP 从虚拟化、并发和持久性三条主线展开操作系统原理，但这些内容都隐含了一个基本前提：**内核已经被加载到内存，并且正在正常运行**。
...
因此，`main`、标准输出和程序退出都不是程序天然具备的能力，而是建立在既有操作系统和运行时环境之上的抽象。

内核启动时不存在一个更高层的操作系统来提供这些服务。本实验希望你借助 QEMU 和 OpenSBI，显式建立一条最小内核运行链路。
...
在 OSTEP 中，“程序由操作系统加载到内存”通常描述的是操作系统已经运行之后的场景。对于 Lab1，操作系统内核本身就是第一个需要被加载和启动的程序，因此不能假设已经存在一个更高层的操作系统来完成这项工作。
