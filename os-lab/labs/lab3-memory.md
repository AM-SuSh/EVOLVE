...
| 变体        | 你在 `mm.rs` 里看到什么                                                                             | 要你做什么                                                                                    |
| --------- | -------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| **fill**  | `map_elf_and_stack` 末尾先调用 `strip_user_bit_for_exercise`，再调用 `restore_user_bit`；后者是 `todo!()` | 按函数上方「思路提示」实现 `restore_user_bit`：在与 strip 相同的 VA 范围上，给「有映射但缺 U」的页 `remap` 回「原 R/W/X + U」 |
| **debug** | 文件头写明现象；`map_elf_and_stack` 在映射栈之前有一段遍历 ELF 区并改权限的代码                                         | 按「现象 → 假设 → 最小实验」排查：对比 ELF 区与用户栈最终各带了哪些位，修好后须看到完整四条输出断言                                  |
...
- 看到 `【Lab3 任务：fill】`：实现 `restore_user_bit`（上方有思路提示；`strip_user_bit_for_exercise` 请勿改）；
- 看到 `【Lab3 任务：debug】`：按文件头的现象与排查步骤，定位并修复 `map_elf_and_stack` 附近的权限问题。
