...
5. **建议先读书**：OSTEP 第 13–16 章（地址空间、地址转换、分段）与第 18 章分页。Lab3 是**内存虚拟化**的核心实验；对应 feature 为 `lab3`（依赖 `lab2`）。
...
打开 `kernel/src/mm.rs` 文件头：

- 出现 `【Lab3 任务：补全】` → **fill**：练习准备已去掉用户区 U 位，实现 `restore_user_bit` 遍历并 remap 补回 U；
- 出现 `【Lab3 任务：排错】` 或 PLANTED BUG → **debug**：用户页缺 `U` 位，按现象→假设排查并修复。
...
> 教师下发的 **fill / debug** 变体是本实验主任务（见任务一）。下列修改用于加深理解，可在变体通过后再做；做完务必改回并复跑。
