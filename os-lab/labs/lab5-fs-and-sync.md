...
5. **建议先读书**：OSTEP 第 28 章（锁）、第 36 章（I/O）、第 39 章（文件和目录）。带着「程序怎么读文件」「隔离进程如何传字节」「为什么要加锁」三个问题进来即可。Lab5 对应 feature 为 `lab5`（依赖 `lab4`）。
...
- **fill**：补全 `child_reader` / `parent_writer`，自己写出关闭哪一端、如何读写与 wait；
- **debug**：代码结构完整但**关错了端**（例如子进程关掉读端却仍去读），需按现象排查修复。
