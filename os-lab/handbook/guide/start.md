...
## 一、EVOLVE 是什么(最最后再总结一下，强调引导式学习和AI导师，支持教师课程教学辅助使用和学生自学使用)

EVOLVE 是一套面向操作系统课程的教学实验环境，技术栈包括：
...
EVOLVE 的核心设计是：**始终只有一个内核代码库**。通过 Cargo 的 feature按阶段打开功能，让内核像搭积木一样逐步“长出”能力。你不用在好多套互不相关的内核工程之间来回切换，更容易看清操作系统是怎样一层层长起来的。
...
5. **留下进度**  
   在工作台完成可信验证与 3–5 题学习复盘，再把报告与复盘一起提交给老师；老师开放下一层后即可解锁，不需要等待教师验收。
