...
- **收获与复盘**：旧“收获与反思”文本框由 `SocraticReviewPanel` 取代。服务端一次只公开已经提出的问题，逐题记录学生原答、Assessment 评价、最终总结和 transcript。
  - **书页式翻页**：进行中的复盘按“每题一页”展示（题目 + 我的回答 + AI 评价同页），顶部页码条可回看任意已答题；待补证据与最终总结各占一页。「回到当前」一键跳回待作答页；已答页底部有「继续下一题 / 写最终总结」前进按钮。
  - **草稿自动暂存**：回答框与总结框的输入按 `labId + sessionId + reviewId` 存入浏览器 localStorage（提交成功或复盘完成后清除）。折叠学习支持区、切换页签、翻页回看或刷新页面都不会丢失未提交的草稿；换设备或换 review 不恢复。
- **合并提交**：报告正文、附件与复盘记录在同一面板中展示。提交给教师的报告正文只含学生自己撰写的内容，复盘问答不再拼接进正文；复盘作为独立证据随同一 Lab 验收对象展示。`review_completed` 和 `deferred` 均可随报告提交，后者会明确携带延期原因和未解决项。
...
- **完成含义**：自动评分和复盘状态不会自行给出教师验收结论。提交后进入“实验验收”队列，由教师结合报告、复盘、运行证据和自动评分作最终验收。

## 教师实验验收（含评分复核）

统一挂载：`/teacher-review`（`TeacherReview`）。原“评分复核”不再是独立教师入口，评分证据与报告验收在同一页面完成。

- **左栏 · 验收队列**：按“学生 + Lab”列出统一验收对象。队列同时接纳已提交报告和仅有复盘的记录；同一学生同一 Lab 不因报告与复盘产生两条任务。
- **中栏 · 提交与复盘**：合并展示报告正文、附件、苏格拉底问题与原答、逐题评价、最终总结、原始 transcript 和 Tutor 日常对话证据。仅有复盘时显示可信空态并保留证据，报告提交后在原条目上补齐正文。
