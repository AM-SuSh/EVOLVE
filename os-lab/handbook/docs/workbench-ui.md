...
桌面端为「左手册 + 右实践区（上：工作区；下：学习支持）」布局；工作区内部为 VS Code 式「**左侧文件栏通栏到底** | 右侧（编辑器 / 底栏终端）」。

- **拖动**：手册与实践区之间的竖条调整左右宽度；工作区与学习支持之间的横条调整上下高度；编辑器与终端之间的横条只作用于右侧栏
...
> 无独立 `BottomDock.vue`：底部页签由 `LabWorkspace` 经 `CodePanel` 的 `#dock` 插槽注入，使文件树与底栏同高。
