...
- **合并提交**：报告正文、附件与复盘记录在同一面板中展示。提交给教师时，系统把正文与复盘章节组装为同一个 Lab 验收对象；`review_completed` 和 `deferred` 均可随报告提交，后者会明确携带延期原因和未解决项。
- **预览最终稿**：与导出 / `POST /reports` 使用同一份 Markdown，包含报告正文和当前可提交的复盘记录，避免学生预览与教师所见不一致。
