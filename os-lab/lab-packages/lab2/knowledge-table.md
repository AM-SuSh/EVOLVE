...
| os.trap.context-switch | 上下文保存恢复 | sepc/sstatus/sscratch、双栈 | `TrapContext`, `trap.asm` | 任务二 Q1–Q2、Q6；任务三注释 csrrw | 正常返回 / 破坏后崩溃 | 线程切换、信号帧 | 在用户栈保存；以为 sepc 已是下一条 |
...
| os.sched.task-state | TCB 与状态机 | 软件状态 Ready/Running/Exited | `task.rs` | 任务二 Q5、Q8；debug 变体 | All exited；task_switch | 进程/线程阻塞态 | yield=exit |
| os.sched.round-robin | 协作轮转 | 从 Ready 集选取 | `find_next_task`, `yield.rs` | 任务二 Q7；fill 变体；观察扫描起点 | Yield round×5 | 时间片、协程调度 | 恒选 0；不更新 current |
