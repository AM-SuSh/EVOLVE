# 成员 A · 7 日交付索引

更新：2026-07-31  
对照计划：仓库外 / 工作区《os-lab 三人小组 · 7 日实施计划》

## 本周交付（按日）

| Day | 任务 | 落点 | 状态 |
| --- | --- | --- | --- |
| 1 | 规格未进产品缺口表 + 5 步演示 | `MEMBER-A-7DAY/day1-gap-table.md` | ✅ |
| 2 | Lab2 检查点机器表 | `lab2/checkpoints.yaml`（对照 `checkpoints.md`） | ✅ |
| 3 | 5 条应拒/应放行金标准 | `learning/tutor-golden-dialogues-lab2.json` | ✅ |
| 4 | 量规 v2 冻结 + 复核对齐 | `learning/rubric-v2-frozen.md` + 既有 `teacher-review-gates.md` | ✅ |
| 5 | 补救变式装箱；OPRE/路径文案 | `lab2/variants/remedial/PACKAGE.md`；`lab2/knowledge-path.md`；`visualization/opre-copy-final.md` | ✅ |
| 6 | 变式教学验收勾选 | `lab2/variants/remedial/TEACHER_ACCEPTANCE.md` | ✅（工程/体验签字待 B/C） |
| 7 | 5 人可用性最小脚本 | `learning/usability-script-5person.md` | ✅（真人试用待安排） |

## 仍非 A 单人可关闭

| 项 | 依赖 |
| --- | --- |
| 门控/评分 API、金标准挂 CI | 成员 C |
| 证据条、OPRE/路径 UI、复核队列、向导 | 成员 B |
| 组织 5 人实测与数据修订 | 课程排期 + 真人 |
| remedial 正式 publish 到班级 | C lint/dry-run + 教师操作 |

## 给 B / C 的第一入口

- **C 加载**：`lab2/checkpoints.yaml`、`learning/tutor-golden-dialogues-lab2.json`、`learning/rubric-v2-frozen.md`  
- **B 文案**：`lab2/knowledge-path.md`、`visualization/opre-copy-final.md`  
- **发布前**：`lab2/variants/remedial/TEACHER_ACCEPTANCE.md`
