...
> 本报告总结自研操作系统教学实验环境 os-lab 的设计思路、与 AI 合作的实现过程，以及学习效果评估。
>
...
为达成目标，os-lab 做了三个关键的架构决策（详见 [architecture.md](architecture.md)）：
...
- **成员 C**（文档与评估，即本报告作者）：与 AI 合作编写 5 个 lab 的实验指导、答案、习题、三方对比报告、设计总结。
...
os-lab 的 5 个 lab 覆盖了操作系统三大核心主题：
...
【待真实学习数据验证】基于设计分析，建议的学习路径：**os-lab 入门（建脉络）→ xv6 深化（广覆盖）**。os-lab 的渐进式+问题驱动适合打地基，xv6 的工程化+广覆盖（11 lab 含网络/mmap）适合建高楼。两者互补，避免初学者被 xv6 的 C 语言+庞大代码量劝退。

## 四、创新点与差异化价值

os-lab 相对参考环境（tg-rcore-tutorial）和本校环境（xv6-riscv）的核心创新（详见 [comparison.md](comparison.md) 第四节）：
...
2. **文件系统是教学简化版**：内嵌只读文件，非真实磁盘 FS。
3. **调度是简化版**：批处理/轮转，非完整抢占式调度；lab2 的 yield 在批处理下曾只轮转一次（lab3 后改善）。
4. **学习效率评估基于设计推断**：【待真实学生使用数据补充验证】。
...
| 完整性 | 20% | 中上 | 5 个 lab 覆盖 OS 三大主题；局限是文件系统简化、无网络/mmap |
| 代码质量 | 25% | 中上 | Rust 内存安全、6 crate 组件化、9 个单元测试；局限是部分 unsafe 用法、警告未全清 |
| 文档完整性 | 25% | 强 | 5 lab 指导 + 5 答案 + 5 习题 + 架构/对比/设计/协作四份报告 + overview 总览，文档体系完整 |

## 七、总结

os-lab 是一套面向 OS 初学者的自研教学实验环境，通过"渐进式单内核 + 精简组件化 + 问题驱动文档 + Rust 内存安全"四大设计，降低了 OS 学习的门槛，让学生能清晰看到内核从裸机到完整系统的演进脉络。
