...
### 本轮代码实现：复盘提交语义与规则/Agent 融合评分收口（2026-08-14）

- **完成条件与回答质量解耦**：新复盘固定生成 3-5 道主问题；学生完成这些问题的作答即进入 `review_completed`。回答是否完全正确只影响“收获与反思”评分，不再把 `passed`、追问次数或全部答对作为报告提交和下一 Lab 的门禁。
- **可信验证不再受时间间隔影响**：复盘与报告提交复用同一学生、同一 Lab 已经通过的可信验证记录；不要求复盘前后重新运行，也不因工作区测试和复盘间隔较长而失效。下一 Lab 的服务端条件统一为“教师已开放 + 前一 Lab 可信验证通过 + 前一 Lab 报告与复盘已联合提交”。
- **报告与复盘联合提交**：复盘 transcript、逐题 Assessment 评价和作答表现摘要先与学生的报告草稿关联保存。只有学生点击“提交给老师”后，报告正文、附件和复盘记录才形成同一份正式提交；教师队列与教师复盘接口都以正式 `report_submitted` 记录为可见性边界。
- **评分 Agent 正式接入**：`POST /assessment` 先生成规则基线，再把学生行为轨迹、可信证据、过程细项和逐题复盘表现交给独立评分 Agent；Agent 成功时按规则 60% + Agent 40% 形成融合分，模型不可用或证据不足时明确降级为 `rule-only`，融合分等于规则基线。
- **删除独立结果维度**：可信运行和命名断言已经通过 `V1`“完成可信验证”与 `I1`“失败后迭代并复验”进入过程分，不再用 Lab2 的 Hello、Power、Yield、全部退出等断言重复形成 `R1-R4` 结果分。规则基线改为过程 75% + 收获与反思 25%，评价版本升级为 `rubric-v3.3.0`。
- **逐题反思评分**：结构化复盘按每个主问题链动态生成 `RQ1-RQn`；首答完成计 2 分、追问后完成计 1 分、已回答但仍可完善计 0 分。历史复盘缺少题面时，从同一 `reviewId` 的 `review_question_asked` 服务端事件恢复原问题。
- **学生端展示去重**：学习评价只保留“过程”和“收获与反思”两个维度，不再展示独立“结果”或大量运行标签；已完成复盘继续使用书页视图，仅展示问题、学生回答和 AI 导师评价，不再重复渲染整段 Markdown。
- **掌握画像与复核门控适配**：掌握度映射和教师复核门控改为识别动态 `RQ*` 反思项；规则基线与评分 Agent 差异达到阈值时进入硬复核，避免旧固定项缺失造成误判。
