...
- 报告正文保留，“收获与反思”改为专门复盘对话区；提交时将报告正文、附件、复盘问答、逐题评价和学生最终总结合并为同一份实验提交。
...
1. **一个提交对象**：学生提交的不只是报告正文。一个 Lab 的验收对象由报告正文与附件、苏格拉底复盘 transcript、逐题 Assessment 评价、最终总结及证据引用共同组成。`deferred` 复盘可以随报告提交，但保留未解决项和延期原因，不因此自动完成 Lab。
2. **一个教师入口**：教师从“实验验收”进入统一队列。只有复盘、尚无报告的学生也进入队列供教师提前查看；报告提交后，同一学生、同一 Lab 更新为完整提交，不再生成另一条割裂记录。
3. **中栏合并阅读**：选中学生后，中栏连续展示报告正文、附件和完整复盘记录，包括问题、学生原答、评价、遗漏点、正确推理、最终总结、 transcript 与 Tutor 日常对话证据。
