...
对应赛题第 1 条技术指标（赛题基线见 [README.md](../README.md)）：在官方参考教学环境中完成 5 个基础实验 **练习（exercise）**。本文同时记录锁定基线下的 **base 模式** 测试结果。
...
在已激活实验环境的终端中，进入对应章节目录，将 `cargo run` 输出管道至 `tg-rcore-tutorial-checker --ch <N>` 进行判定。ch5、ch6、ch8 须设置 `CHAPTER=-5`、`-6`、`-8` 后重新编译运行。
